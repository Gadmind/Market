        ...
        <ul id="leftMenu">
            <c:forEach items="${productCategoryVoList}" var="temp">
                <li>
                    ...
                    <div class="zj">
                        <div class="zj_l">
                            <c:forEach items="${temp.productCategoryVo}" var="vo">
                                <div class="zj_l_c">
                                    <h2>
                                        <a href="${ctx}/ProServlet?mark=queryProductList&category=${vo.productCategory.id}&level=2">${vo.productCategory.name}</a>
                                    </h2>
                                    <c:forEach items="${vo.productCategoryVo}" var="vo2">
                                        <a href="${ctx}/ProSerlvlet?mark=queryProductList&category=${vo2.productCategory.id}&level=3">${vo2.productCategory.name}</a>
                                    </c:forEach>
                                </div>
                            </c:forEach>
                        </div>
